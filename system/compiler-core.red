Red [
	Title: "Shared Red/System compiler implementation"
...
system-dialect: context [
	...
	options-class: context [
		...
		packager:			none						;-- optional post-link application packager
	]
	...
	compiler: context [
      ...
      (to-word "%")	[2	op		- [a [number!] b [number!] return: [number!]]]		;-- remainder (real syntax: %)
		...
		calc-line: has [idx prev p header pos mark][
			header: head pc
			;-- Compiler-generated blocks do not necessarily carry the loader's
			;-- hidden line table. They still need to be accepted in debug builds.
			unless integer? header/1 [return 1]
		...
		not-typed?: func [name [word!] /local pos][
			all [
				...
				pos: find-local-spec/from name next pos
					...
					io [
						switch/default path/3 [
							...
							write [
								...
								emitter/target/emit-restore-last
		...
		find-local-spec: func [
			name [word! set-word!]
			/from series [block!]
			/local pos kind
		][
			pos: either from [series][locals]
			kind: type?/word name
			while [all [pos pos: find pos name]][
				if kind = type?/word pos/1 [return pos]
				pos: next pos
			]
			none
		]

		select-local-spec: func [name [word! set-word!] /local pos][
			all [pos: find-local-spec name pos/2]
		]

		local-variable?: func [name [word!]][
			find-local-spec name
		]
		...
		variadic?: func [value][
			to logic! all [issue? value find [#variadic #typed #custom] value]
		]
		...
		int64-literal-info: func [value /local s kind payload neg? digits type][
			unless issue? value [return none]
			s: form value
			if s/1 = #"#" [remove s]
			case [
				find/match s "i64-"  [kind: 'i64  payload: skip s 4]
				find/match s "u64-"  [kind: 'u64  payload: skip s 4]
				find/match s "u64h-" [kind: 'u64h payload: skip s 5]
				find/match s ".i64:" [kind: 'i64  payload: skip s 5]
				find/match s ".u64:" [kind: 'u64  payload: skip s 5]
				find/match s ".u64h:" [kind: 'u64h payload: skip s 6]
				true [return none]
			]
			switch kind [
				i64 [
		...
		int64-hex: func [value type [word!] /local info hex neg?][
			case [
				...
				integer? value [
					neg?: negative? value
					hex: decimal64-to-hex form absolute value neg?
			...
			switch/default type?/word value [
				...
				paren!	 [
					either all [
						...
						any [
							all [
								type: get-type value/1
								find [function! c-string!] first type
							]
							find value get-word!
							find value string!
						]
		...
		cast: func [obj [object!] /quiet /local value ctype type][
			...
			switch ctype/1 [
				...
				int8! uint8! int16! uint16! int32! uint32! [
					if find [byte! logic!] type/1 [value: make integer! value]
				]
				integer! [
				...
				float! float32! [
					if type/1 = 'integer! [
						value: either all [obj/keep? ctype/1 = 'float32!][
							head reverse debase/base to string! to-hex value 16
						][
							to float! value
						]
					]
		...
		preprocess-array: func [list [block!] /local p s v][
			; Bind p to each token. Resolve enum words to integers before store.
			; Evaluate paren expressions after resolving enums inside them.
			parse list [
				some [
					p: ['true | 'false] (p/1: do p/1)
					| p: 'null (p/1: 0)
					| p: word! (check-enum-symbol/strict p)
					| p: paren! (
						parse p/1 [
							any [
								array-expr-keywords
								| s: word! (check-enum-symbol/strict s)
								| skip
							]
						]
						change p do p/1
					)
					| string! | char! | integer! | float! | issue! | get-word!
		...
		fetch-into: func [								;-- compile sub-block
			...
			;-- Parent cursor in Red function local save-pc (Red-stack GC root).
			;-- Matches historical Red port; avoids frame-stack series corruption.
			if root [
			...
			if none? save-pc [
				throw-error "internal: fetch-into restored pc is none"
			]
			next pc: save-pc
		]
		...
		encode-pointers: func [name specs [block!] /metadata fspec [block!] /local list offset b][
			...
			if verbose > 5 [
				...
				foreach n list [
					if integer? n [
						print enbase/base b: debase/base to string! to-hex n 16 2
						print b
					]
				]
			]
			...
			unless parse defs [
				some [
					pos: set lib string! (
						...
						unless any [suffix? to-file lib  external-linker/framework? lib][
							lib: external-linker/resolve-libname lib job/format job/static-link?
						]
						new?: no
					...
					pos: into [
						...
						if all [empty-import?  not external-linker/library? lib][throw-error err]
						if new? [repend imports [lib list]]
						if external-linker/library? lib [external-linker/register job lib script cc]
					)
				]
			][throw-error err]
		]
		...
		comp-directive: has [body][
			switch/default pc/1 [
				...
				#build-date[change pc mold now/utc]	;-- UTC
		...
		comp-as: has [ctype ptr? expr type k?][
			...
			if path? ctype [ctype: path-to-word ctype]
		...
		comp-either: has [expr e-true e-false c-true c-false offset t-true t-false ret mark name fspec][
			...
			if any [
				all [
					...
					ret: select-local-spec return-def	;-- and if function returns something
			...
			parse body rule: [
				while [
					'loop pos: (
						unless issue? pos/-2 [
							loops: loops + 1
							insert pos append copy <L> loops
							pos: next pos
						]
					)
			...
			either all [
				...
				case [
					value: system-reflexion? path [
						either path/2 = 'words [
							return comp-word value 1 ;-- re-route to global word resolution
						][
							pc: next pc
						]
					]
					...
					'else [
						comp-word path/1 2					;-- check if root word is defined
		...
		comp-word: func [
			...
			/local entry local? spec type path-mode? check-mode?
		][
			path-mode?: mode = 2
			check-mode?: mode = 3
			if zero? mode [
			...
			case [
				...
				any [
					all [
						...
						any [
							all [						;-- block local function pointers
								block? type: select-local-spec name
								type: resolve-aliased type
					...
					unless check-mode? [also name pc: next pc]
				]
				all [
					locals
					block? type: select-local-spec name
				...
				all [
					...
					unless check-mode? [comp-func-args name entry]
				]
		...
		arm64-hfa-result?: func [spec [block!] /local ret][
			all [
				...
				first target-homogeneous-aggregate? emitter/target ret
			]
		]
					...
					job/target = 'ARM64 [
						all [
							...
							not either result [
								...
								all [aggregate first target-homogeneous-aggregate? emitter/target aggregate-type]
							]
						]
					]
					job/target = 'X86-64 [
		...
		reserve-call-struct-temps: func [
			list [block!] types spec [block!]
			/local scan-types expr struct-type struct-slots struct-size temp-slots
		][
			temp-slots: 0
			if all [types find [X86-64 ARM64] job/target][
				...
				if positive? temp-slots [
					target-reserve-call-struct-temps emitter/target temp-slots
				]
			]
		]
		...
		comp-call: func [
			...
			][emitter/target/emit-stack-align-prolog args spec]

			if args/1 <> #custom [
				fspec: functions/:name
				type: second fspec
				either type <> 'op [
					all [
						not empty? list
					...
					if find [X86-64 ARM64] job/target [
						reserve-call-struct-temps list types spec
					]
			...
			either res [
				compiler-api/set-last-type res
			][
				if type: select functions/:name/4 return-def [
			...
			][emitter/target/emit-stack-align-epilog args]
		...
		comp-expression: func [
			...
			either block? expr [
				...
				last-type: either not any [
					...
					either all [
						boxed
						not float? unbox expr
						not float? boxed/data
					][
			...
			if all [									;-- clean FPU stack when required
				...
				any [
					...
					all [
						...
						not find-local-spec return-def ;@@ works for non-terminal expressions only
					]
				]
		...
		fetch-expression: func [
			...
			set/any 'expr switch/default type?/word pc/1 [
				set-word!	[comp-assignment]
				word!		[comp-word pc/1 0]
				...
				issue!		[either any [#"." = first to-string pc/1 int64-literal-info pc/1][do pass][comp-directive]]
			][
		...
		comp-block: func [/final /only /local expr mark save-pc][
			block-level: block-level + 1
			save-pc: pc									;-- Red-stack local roots parent cursor
			pc: pc/1
			...
			if none? save-pc [
				throw-error "internal: comp-block restored pc is none"
			]
			pc: next save-pc
		...
		comp-dialect: has [expr][
			...
			while [not tail? pc][
				case [
					all [
						issue? pc/1
						not int64-literal-info pc/1
						#"." <> first to-string pc/1
					][comp-directive]
					all [
						set-word? pc/1
					...
					paren? pc/1 [
						...
						set/any 'expr fetch-expression/final/keep none
					]
	...
	make-job: func [opts [object!] file [file!] /local job blk pos][
		...
		file: last split-path file									;-- remove path
			...
			case [
				none? get in job 'build-basename [
					set in job 'build-basename file
				]
				slash = last get in job 'build-basename [
					append get in job 'build-basename file
			]
		]
		job
	]
	...
	compile-sources: func [
		files [block!] loaded? job-data job [object!] resources [block!]
		/local file src
	][
		if not tail? files [
			file: first files
