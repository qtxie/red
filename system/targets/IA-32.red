...
system-target-IA32: context [
	; Keep this body literal: Red's native compiler binds object methods statically.
	...
	emit: func [bin [binary! char! block!]][
		...
		append emitter/code-buf either char? bin [
			int-to-bin/to-bin8 to integer! bin
		][bin]
	]
	...
	patch-opcode: func [
		...
		and-mask [binary!]
		or-mask [binary! none!]
		/local result index limit
	][
		result: copy opcode
		limit: min length? result length? and-mask
		repeat index limit [
			poke result index ((pick result index) and (pick and-mask index))
		]
		if or-mask [
			limit: min length? result length? or-mask
			repeat index limit [
				poke result index ((pick result index) or (pick or-mask index))
			]
		]
		result
	]
	...
	homogeneous-floats?: func [spec [block!]][
		reduce [no 0]
	]

	emit-push-struct-ref: func [slots [integer!]][
		compiler-api/throw-error "struct reference arguments are unsupported on IA-32"
	]
		...
		if select emitter/symbols sym [
			...
			until [
				...
				if zero? retry: retry - 1 [
					compiler-api/throw-error "Unable to create native helper symbol!"
				]
	...
	call-i64-divide: func [name [word!] /local spec mode][
		...
		emit #{B1} emit int-to-bin/to-bin8 mode					;-- MOV cl, mode
	...
	on-global-epilog: func [runtime? [logic!] type [word!]][
		either runtime? [
			...
			either compiler-api/job-value 'need-main? [
				...
				emit #{5D}							;-- POP ebp
				args: switch/default compiler-api/job-value 'OS [
					Syllable [6]
				][7]
	...
	emit-float: func [opcode [binary!]][
		emit either width = 4 [patch-opcode opcode #{F9FF} none][opcode]
	]

	emit-float-arg: func [arg opcode [binary!]][
		emit switch/default first compiler-api/get-type arg [
			float32! [patch-opcode opcode #{F9FF} none]
			integer! [patch-opcode opcode #{F0FF} #{0B00}]
		][
			opcode
		]
	]
	emit-float-variable: func [
		...
		switch type: first compiler-api/get-type name [
			float32! [foreach c codes [set c patch-opcode (get c) #{F9FF} none]]
			integer! [foreach c codes [set c patch-opcode (get c) #{F0FF} #{0B00}]]
		]
	...
	emit-poly: func [spec [block!] /local w value][	;-- polymorphic code generation
		spec: reduce spec
	...
	emit-save-last: does [
		...
		case [
			...
			not compiler-api/any-float? compiler-api/last-type [
				emit #{50}							;-- PUSH eax
			]
		]
	]
	...
	emit-restore-last: does [
		either compiler-api/int64? compiler-api/last-type [
			...
			unless find [float! float64! float32!] first compiler-api/last-type [
				emit #{5A}					   		;-- POP edx
			]
		]
	]
	...
	emit-casting: func [
		...
		case [
			value/type/1 = 'logic! [
				...
				old: width
				either compiler-api/int64? type [
					either alt? [
			...
			all [
				...
				not compiler-api/int64? value/type
				not compiler-api/int64? type
			][
				from: compiler-api/canonical-type type
				to: compiler-api/canonical-type value/type
				from-width: compiler-api/integer-width? from
				to-width: compiler-api/integer-width? to
				signed-src?: compiler-api/signed-integer? from
			...
			all [compiler-api/int64? value/type compiler-api/integer-type? type][
			...
			all [compiler-api/integer-type? value/type compiler-api/int64? type][
				if verbose >= 3 [print [">>>converting from" type/1 "to" value/type/1]]
			]
			...
			all [value/type/1 = 'integer! find [float! float64! float32!] type/1][
				...
				either all [value/keep? type/1 = 'float32!][
					...
					either (compiler-api/job-value 'cpu-version) >= 4.0 [ ;-- Only CPUs with SSE3, >= Pentium 4
	...
	emit-fpu-set: func [
		...
		case [
			...
			masks [
				...
				emit int-to-bin/to-bin32 complement shift/left 1 bit
			]
		]
		either cword [
	...
	emit-overflow-check-shift: func [n [integer!] /local mask bias bits][
		...
		either all [signed? width = 4][				;-- signed bias trick: only meaningful for 32-bit integer!
			...
			;-- int-to-bin/to-bin8 / int-to-bin/to-bin16 / int-to-bin/to-bin32 truncate their input to the matching low N bytes,
			;-- so passing the raw `shift/left -1 (bits - n)` value works directly (no extra mask).
			switch width [
				1 [
					emit #{F6C0}					;-- TEST al, imm8
					emit int-to-bin/to-bin8 shift/left -1 8 - n
				]
				2 [
					emit #{66F7C0}					;-- TEST ax, imm16 (with operand-size prefix)
					emit int-to-bin/to-bin16 shift/left -1 16 - n
				]
				4 [
					emit #{F7C0}					;-- TEST eax, imm32
					emit int-to-bin/to-bin32 shift/left -1 32 - n
				]
			]
		]
	...
	emit-load-int-literal: func [value type [word!] /local hex w][
		w: compiler-api/integer-width? type
		hex: compiler-api/int-literal-hex value type
		either w = 8 [
	...
	emit-load-int64-variable: func [value [word! object!] /local offset spec][
		if object? value [value: compiler-api/unbox value]
		...
		switch type?/word value [
			...
			issue! [
				either spec: compiler-api/int64-literal-info value [
					either all [cast compiler-api/integer-type? cast/type not compiler-api/int64? cast/type][
					...
					emit int-to-bin/to-bin8 pick [4 8] to logic! all [cast cast/type/1 = 'float32!]
				]
			]
			float! [
				set-width any [cast value]
			...
			word! [
				with-width-of value [
					case [
						compiler-api/any-float? compiler-api/get-variable-spec value [
							load-float-variable value
						]
						all [
							offset: emitter/local-offset? value
							'value = last select compiler-api/locals value
			...
			get-word! [
				...
				either any [
					all [
						spec: select compiler-api/functions value
						spec/2 = 'routine
					]
					all [
						emitter/local-offset? value
						'function! = first compiler-api/get-type value
					]
				][
					either alt [
						emit-variable value
	...
	emit-store: func [
		...
		switch type?/word value [
			...
			integer! [
				either compiler-api/int64? compiler-api/get-variable-spec name [
					emit-load-int64-literal value first compiler-api/get-variable-spec name
			...
			issue! [
				either type: compiler-api/int64-literal-info value [
					either compiler-api/int64? compiler-api/get-variable-spec name [
						emit-load-int64-literal value first compiler-api/get-variable-spec name
						emit-store name <last> none
					][
						...
						emit-variable-poly name
							...
							#{C683} #{C783}
							#{C645} #{C745}
						value: to integer! to issue! compiler-api/int-literal-hex value first compiler-api/get-variable-spec name
						emit switch width [
							1 [int-to-bin/to-bin8 value]
							2 [int-to-bin/to-bin16 value]
							4 [int-to-bin/to-bin32 value]
			...
			word! [
				case [
					compiler-api/any-float? compiler-api/get-variable-spec name [
						store-float-variable name
					]
					by-value [
						if slots <= 2 [				 ;-- if > 2, copied already, do nothing
							either offset: emitter/local-offset? name [
								if slots = 2 [
									...
									emit int-to-bin/to-bin32 offset + 4
								]
			...
			binary! [
				either PIC? [
					...
					do store-dword
					either all [binary? value 'float32! = first compiler-api/get-type name][ ;-- `as float32! keep` case
						emit value
					][
	...
	emit-access-path: func [
		...
		unless spec [
			name: to word! path/1
			spec: second compiler-api/resolve-type name
			emit-load name
		]
		if short [return spec]

		saved: width
		type: compiler-api/resolve-type/with path/2 spec
	...
	emit-load-index: func [idx [word!]][
		unless compiler-api/local-variable? idx [idx: compiler-api/resolve-ns idx]
		emit-variable idx
	...
	emit-pointer-path: func [
		...
		either compiler-api/any-float? type/2 [
			opcodes: pick [[						;-- store path opcodes --
				#{DD18}								;-- FSTP [eax]
	...
	emit-store-path: func [
		...
		value-type: either value = <last> [compiler-api/last-type][compiler-api/get-type value]
		size: emitter/size-of? value-type
		pair?: all [size = 8 not compiler-api/any-float? value-type]
		either value = <last> [
			if by-val?: 'value = last compiler-api/last-type [
				slots: emitter/struct-slots? compiler-api/last-type
			...
			either pair? [
				...
				unless all [
					...
					word? path/2
					not object? value
					spec: any [parent second compiler-api/resolve-type path/1]
					type2: select spec path/2
					compiler-api/any-float? type2
		...
		switch type [
			...
			struct! union! [
				...
				type: compiler-api/resolve-type/with path/2 parent
				if all [block? type 'value = last type spec: compiler-api/find-aliased type/1][
	...
	patch-sub-call: func [buffer [binary!] ptr [integer!] offset [integer!]][
		change at buffer ptr int-to-bin/to-bin32 negate offset + 5 - 1
	]
	...
	patch-jump-point: func [buffer [binary!] ptr [integer!] exit-point [integer!]][
		change at buffer ptr int-to-bin/to-bin32 exit-point - ptr - branch-offset-size
	]
	...
	construct-jump: func [
		...
		o: either short? [int-to-bin/to-bin8 o][int-to-bin/to-bin32 o]	;-- make binary signed offset
			...
			either not parity [
				...
				either back? [							;-- in `back?` mode size is adjusted by jxx automatically
					either unord-jumps-to-true? [
						...
						append jump rejoin [#{7A} int-to-bin/to-bin8 length? jcc]	;-- append JP _false, over the Jcc size
	...
	emit-push-struct: func [
		...
		either slots <= 5 [							;-- 5 is the breaking point where the loop takes less opcodes
			...
			emit int-to-bin/to-bin32 slots
			emit #{F3A5}							;-- REP MOVS
		]
	]
	...
	emit-extend-int32: func [type [block! word! integer! none!] /local target][
		...
		if all [
			compiler-api/integer-type? type
			not compiler-api/int64? type
			(compiler-api/integer-width? type) < 4
		][
			target: reduce [either compiler-api/signed-integer? type ['integer!]['uint32!]]
			emit-casting compiler-api/make-action [
				action: 'type-cast
				type: target
				data: <last>
			] no
			compiler-api/set-last-type target
		]
	]

	emit-normalize-return: func [fspec [block!] /local type][
		if all [
			type: select fspec/4 compiler-api/return-def
			compiler-api/integer-type? type
			not compiler-api/int64? type
			(compiler-api/integer-width? type) < 4
		][
			switch compiler-api/integer-width? type [
				1 [emit either compiler-api/signed-integer? type [#{0FBEC0}][#{0FB6C0}]]
				2 [emit either compiler-api/signed-integer? type [#{0FBFC0}][#{0FB7C0}]]
			]
		]
	]
	...
	emit-push: func [
		...
		/cdecl										;-- external call
		/keep
		/local spec type offset conv-int-float? float-type?
	][
		...
		switch type?/word value [
			tag! [									;-- == <last>
				either value = <last> [
					either all [
						any [block? compiler-api/last-type word? compiler-api/last-type]
						compiler-api/int64? compiler-api/last-type
					][
						emit #{52}					;-- PUSH edx
						emit #{50}					;-- PUSH eax
						...
						set-width/type any [all [cast cast/type] compiler-api/last-type]
						emit #{83EC}				;-- SUB esp, 8|4
						emit int-to-bin/to-bin8 width
						emit-float #{DD1C24}		;-- FSTP [esp]
					][
						emit-extend-int32 compiler-api/last-type
						emit #{50}					;-- PUSH eax
					]]
			...
			integer! [
				either all [cast compiler-api/int64? cast/type][
					emit-load-int64-literal value cast/type/1
					compiler-api/set-last-type cast/type
					emit-push <last>
				][
			...
			issue! [
				either type: compiler-api/int64-literal-info value [
					either all [cast compiler-api/integer-type? cast/type not compiler-api/int64? cast/type][
						...
						compiler-api/set-last-type reduce [type/1]
					]
					emit-push <last>
				][
			...
			word! [
				type: compiler-api/get-variable-spec value
				case [
					all [
						'value = last type
						offset: emitter/local-offset? value
			...
			path! [
				emitter/access-path value none
				compiler-api/set-last-type either all [cast not keep][
					emit-casting cast no
					cast/type
				][
					compiler-api/resolve-path-type value
				]
				unless keep [emit-push <last>]
			]
			object! [
				type: compiler-api/get-type value/data
				float-type?: compiler-api/any-float? value/type

				conv-int-float?: any [
					all [float-type?	type/1 = 'integer!]
					all [
				...
				either keep [emit-casting value no][
					unless all [
						float-type?
						any [float? value/data compiler-api/any-float? type]
					][
	...
	emit-bitshift-op: func [name [word!] a [word!] b [word!] args [block!] /local c value][
		...
		if all [compiler-api/overflow-check? name = first [<<] b = 'imm find [1 2 4] width][
			emit-overflow-check-shift compiler-api/unbox args/2
		]
		switch b [
			ref [
		...
		if b = 'imm [
			...
			value: compiler-api/unbox args/2
			unless all [0 <= value value <= c][
				compiler-api/backtrack name
				compiler-api/throw-error rejoin [
	...
	emit-bitwise-op: func [name [word!] a [word!] b [word!] args [block!] /local code][
		...
		switch b [
			imm [
				either all [name = 'and 255 = compiler-api/unbox args/2][
					emit #{0FB6C0}					;-- MOVZX eax, al		; == AND eax, 0xFF
				][
					emit code/1						;-- <OP> eax, value
					emit int-to-bin/to-bin32 compiler-api/unbox args/2
				]
			]
			ref [
	...
	emit-math-op: func [
		...
		arg2: compiler-api/unbox args/2
		load?: not all [
			object? args/2
			b = 'ref
			args/2/type/1 = 'integer!
			compiler-api/any-float? compiler-api/get-variable-spec args/2/data
		]

		if all [
			find [+ -] name							;-- pointer arithmetic only allowed for + & -
			type: compiler-api/resolve-aliased compiler-api/resolve-expr-type args/1
			not compiler-api/any-pointer? compiler-api/resolve-expr-type args/2	;-- no scaling if both operands are pointers
			scale: switch type/1 [
			...
			either compiler-api/literal? arg2 [
				...
				either b = 'reg [
					...
					emit-load args/2
					if object? args/2 [implicit-cast compiler-api/unbox args/2 no]
				]
		...
		switch name [
			+ [
				...
				switch b [
					imm [
						emit-poly either all [arg2 = 1 not compiler-api/overflow-check?][	;-- INC doesn't set CF; skip under tracking
							[#{FEC0} #{40}]			;-- INC rA
						][
			...
			- [
				...
				switch b [
					imm [
						emit-poly either all [arg2 = 1 not compiler-api/overflow-check?][	;-- DEC doesn't set CF; skip under tracking
							[#{FEC8} #{48}]			;-- DEC rA
						][
			...
			* [
				...
				switch b [
					imm [
						either all [
							not compiler-api/overflow-check?	;-- skip SAL trick under tracking (OF undefined for n>1)
							not zero? arg2
			...
			/ [
				op-poly: [
					either signed? [
						...
						if all [compiler-api/overflow-check? width = 4][
	...
	emit-load-int64-right: func [arg /local type][
		type: compiler-api/get-type arg
		switch/default type?/word compiler-api/unbox arg [
			integer! issue! [
				emit-load-int64-literal compiler-api/unbox arg type/1
			]
			word! [
				emit-load-int64-variable compiler-api/unbox arg
			]
			object! [
				emit-load arg/data
				emit-casting arg no
	...
	emit-int64-shift: func [name [word!] count [integer!] /local left? unsigned?][
		unless all [0 <= count count <= 63][
			compiler-api/throw-error "a value in 0-63 range is required for this shift operation"
		]
		...
		case [
			...
			left? [
				either count < 32 [
					emit #{0FA4C2} emit int-to-bin/to-bin8 count	;-- SHLD edx, eax, count
					emit #{C1E0} emit int-to-bin/to-bin8 count		;-- SHL eax, count
				][
					emit #{89C2}						;-- MOV edx, eax
					emit #{31C0}						;-- XOR eax, eax
					if count > 32 [emit #{C1E2} emit int-to-bin/to-bin8 (count - 32)]
				]
			]
			unsigned? [
				either count < 32 [
					emit #{0FADD0} emit int-to-bin/to-bin8 count	;-- SHRD eax, edx, count
					emit #{C1EA} emit int-to-bin/to-bin8 count		;-- SHR edx, count
				][
					emit #{89D0}						;-- MOV eax, edx
					emit #{31D2}						;-- XOR edx, edx
					if count > 32 [emit #{C1E8} emit int-to-bin/to-bin8 (count - 32)]
				]
			]
			'else [
				either count < 32 [
					emit #{0FADD0} emit int-to-bin/to-bin8 count	;-- SHRD eax, edx, count
					emit #{C1FA} emit int-to-bin/to-bin8 count		;-- SAR edx, count
				][
					emit #{89D0}						;-- MOV eax, edx
					emit #{C1FA1F}						;-- SAR edx, 31
					if count > 32 [emit #{C1F8} emit int-to-bin/to-bin8 (count - 32)]
				]
			]
		]
	]
	...
	emit-integer-operation: func [name [word!] args [block!] /local a b sorted? left right saved][
		...
		either all [
			object? args/2
			find [imm reg] b
			not compiler-api/integer-type? args/2/type	;-- skip explicit casting to integer! (implicit)
		][
			...
			unless compiler-api/any-pointer? compiler-api/get-type args/2 [
		...
		if all [compiler-api/overflow-check? find [+ - *] name][
	...
	emit-float-comparison-op: func [
		...
		either (compiler-api/job-value 'cpu-version) >= 6.0	[	;-- support for FCOMI* only with P6+
	...
	emit-float-math-op: func [
		...
		all [
			find [+ -] name
			any [
				compiler-api/any-pointer? compiler-api/get-type args/1
				compiler-api/any-pointer? compiler-api/get-type args/2
			]
			compiler-api/throw-error "unsupported operation with float numbers"
		]
	...
	emit-float-operation: func [
		...
		switch a [									;-- load left operand on FPU stack
			imm [
				spec: emitter/store-value none args/1 compiler-api/get-type args/1
				either PIC? [
			...
			reg [
				...
				if object? args/1 [
					...
					set-width/type compiler-api/set-last-type args/1/type
				]
		...
		switch b [									;-- load right operand on FPU stack
			imm [
				spec: emitter/store-value none args/2 compiler-api/get-type args/2
				either PIC? [
	...
	emit-cdecl-pop: func [spec [block!] args [block!] /local size slots][
		...
		if all [
			spec/2 = 'syscall
			(compiler-api/job-value 'syscall) = 'BSD
		][
			size: size + stack-width				;-- account for extra space
		]
		if compiler-api/variadic? args/1 [
		...
		all [
			spec/2 = 'import
			(compiler-api/job-value 'OS) <> 'Windows
			slots: emitter/struct-slots?/check spec/4
			not all [
				find [macOS FreeBSD NetBSD] (compiler-api/job-value 'OS) ;-- for those OS,
	...
	patch-call: func [code-buf rel-ptr dst-ptr][
		change										;-- CALL NEAR disp size
			at code-buf rel-ptr
			int-to-bin/to-bin32 dst-ptr - rel-ptr - ptr-size
	]
	...
	emit-argument: func [arg fspec [block!]][
		...
		either all [
			...
			any [
				arg/type = 'logic!
				compiler-api/integer-type? arg/type
				compiler-api/integer-type? compiler-api/get-type arg/data
			]
			not path? arg/data
		][
			unless block? arg [emit-load arg]		;-- block! means last value is already in eax (func call)
			emit-casting arg no
			compiler-api/set-last-type arg/type		;-- for inline unary functions
			emit-push <last>
		][
	...
	emit-call-syscall: func [args [block!] fspec [block!] attribs [block! none!]][
		switch compiler-api/job-value 'syscall [
		...
		switch compiler-api/job-value 'syscall [
			...
			Linux [
				...
				if PIC? [
					...
					emit int-to-bin/to-bin8 negate (fspec/1 + 1) * 4
				]
			]
		]
	]
	...
	emit-call-import: func [args [block!] fspec [block!] spec [block!] attribs [block! none!] /local cdecl?][
		...
		emit-reloc-addr second emitter/target/last-red-frame ;-- save frame pointer for later frames chaining

		cdecl?: fspec/3 = 'cdecl
		if all [compiler-api/variadic? args/1 not cdecl?][emit-variadic-data args]

		either (compiler-api/job-value 'OS) = 'macOS [
			either PIC? [
				emit #{8D83}						;-- LEA eax, [ebx+disp]	; PIC
			][
	...
	emit-stack-align-prolog: func [args [block!] fspec [block!] /local offset extra][
		if compiler-api/job-value 'stack-align-16? [
			...
			offset: 4 + either compiler-api/variadic? args/1 [ ;-- account for saved edi
				all [
					args/1 = #variadic
					fspec/3 <> 'cdecl
			...
			unless zero? offset: offset % 16 [
				...
				emit int-to-bin/to-bin8 16 - offset
			]
			emit #{57}								;-- PUSH edi
		]
	]
	...
	emit-open-catch: func [body-size [integer!]][
		...
		emit #{58}									;-- POP eax
		emit #{05}							 		;--	ADD eax, <offset>
		emit int-to-bin/to-bin32 body-size + 9					;-- account for catch-frame opcodes after `CALL next`
	...
	emit-prolog: func [name [word!] locals [block!] bitmap [integer!] /local fspec attribs offset locals-size cb?][
		...
		either any [PIC? none? emitter/target/last-red-frame][
			emit #{6A00}						;-- PUSH 0		; placeholder
		][
			emit #{FF35}						;-- PUSH [last-red-frame]
			emit-reloc-addr second emitter/target/last-red-frame
		]
		...
		if cb? [
			...
			if PIC? [
				...
				emit int-to-bin/to-bin32 emitter/tail-ptr + 1 - offset	;-- +1 adjustment for CALL first opcode
	...
	emit-epilog: func [
		...
		if slots [
			SysVABI?:  all [(compiler-api/job-value 'OS) = 'Linux fspec/3 = 'cdecl]
			macOSABI?: all [(compiler-api/job-value 'OS) = 'macOS fspec/3 = 'cdecl]
			case [
				all [not SysVABI? slots = 1][
					emit #{8B00}					;-- MOV eax, [eax]
					if all [macOSABI? type: compiler-api/is-small-struct-float? fspec/4 type/1 = 'float32!][
						emit #{50}					;-- PUSH eax
						emit #{D90424}				;-- FLD dword [esp]		; load as 32-bit
						emit #{83C404} 				;-- ADD esp, 4
					]
				...
				all [not SysVABI? slots = 2][
					emit #{8B5004}					;-- MOV edx, [eax+4]
					emit #{8B00}					;-- MOV eax, [eax]
					if all [macOSABI? type: compiler-api/is-small-struct-float? fspec/4 find [float! float64!] type/1][
						emit #{52}					;-- PUSH edx
						emit #{50}					;-- PUSH eax
						emit #{DD0424}				;-- FLD qword [esp]		; load as 64-bit
				...
				'else [
					...
					emit int-to-bin/to-bin8 vars/2
			...
			if clean-hidden-ptr?: all [
				tag? emitter/stack/1
				any [SysVABI? all [slots > 2 (compiler-api/job-value 'OS) = 'macOS]]
			][
				emit #{8B45}					    ;-- MOV eax, [ebp+<ptr>]
				emit int-to-bin/to-bin8 emitter/stack/2
			]
		]
		if any [
			fspec/5 = 'callback
			...
			emit int-to-bin/to-bin32 negate offset + 12		;-- account for 3 saved regs
			emit #{5F}								;-- POP edi
			emit #{5E}								;-- POP esi
			emit #{5B}								;-- POP ebx

			unless any [PIC? none? emitter/target/last-red-frame][
				emit #{8DA5}							;-- LEA esp, [ebp-<offset>]
				emit int-to-bin/to-bin32 negate locals-offset		;-- points to last-red-frame saved slot
		...
		either any [
			...
			either compiler-api/check-variable-arity? locals [
				...
				emit int-to-bin/to-bin16 round/to/ceiling args-size 4
			]
		]
	]
]
