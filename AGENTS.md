...
- Current baseline: `build/self-hosting/merge-red64/hybrid-compiler208.exe`
  (207->208, output 6443520 bytes; 206, 207 and 208 all emit 6443520, so the
  chain is at a fixed point). 208 carries the **redundant-cast warning**
  work: warnings now print the location that raised them --
  `*** Warning: type casting from struct! to struct! is not necessary
  ...
  Compiling `red-bootstrap-hybrid.red` went from 361 warnings to 3. The
  three left are generated by the `#call` directive: `process-call-directive`
  emits `red/stack/push as cell! <arg>` because it cannot know the
  Red/System type of the argument, so when the argument already is a cell
  the cast is redundant and is (rightly) reported. Dropping the `as cell!`
  was tried: the build dies with "native codegen rejected invalid RSIR",
  because a call site does not accept a struct alias whose layout merely
  matches the parameter's. The cast is load-bearing.
  Also at 208: `emit-routine` no longer emits `as <value-type> r_arg`
