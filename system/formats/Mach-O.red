...
system-format-MachO: context [
	...
	segment-command: make-struct [
		...
		segname1		[float!]	; char segname[16]
		segname2		[float!]
	...
	section-header: make-struct [
		sectname1		[float!]	; char sectname[16]
		sectname2		[float!]
		segname1		[float!]	; char segname[16]
		segname2		[float!]
	...
	resolve-data-refs: func [job [object!] /local cbuf dbuf data code seg-list][
		cbuf: job/sections/code/2
		dbuf: job/sections/data/2
		seg-list: segments
		while [not tail? seg-list][
			switch seg-list/2 [
				__TEXT [code: get-section-addr '__text]
				__DATA [data: seg-list/3]
			]
			seg-list: skip seg-list 10
		]
	...
	build-uthread-command: func [job [object!] spec [block!] /local ut][
		...
		ut/eip:			(get-section-addr '__text) + any [external-linker/cpp-entry 0]
		form-struct ut
	]
	...
	build: func [
		job [object!]
		/local
			base-address dynamic-linker out sections data buffer seg
	][
		...
		if find job/sections 'ehframe [
			external-linker/ehframe-base: get-section-addr '__eh_frame
			external-linker/rewrite-macho-ehframe			;-- re-encode the relocless pcrel
