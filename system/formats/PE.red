...
system-format-PE: context [
	default-icon: compiler-assets/default-icon
	...
	on-file-written: func [
		job [object!]
		file [file!]
		/local offset buffer value sum position size word
	][
		if system/platform = 'Windows [
			...
			while [position <= size][
				word: either all [
					...
					(to integer! buffer/:position) + either position < size [
						(to integer! buffer/(position + 1)) * 256
					][0]
				]
				sum: sum + word
				sum: (sum and 65535) + shift/logical sum 16
				position: position + 2
			]
			sum: (sum and 65535) + shift/logical sum 16
			sum: (sum and 65535) + shift/logical sum 16
			value: sum + size
			change/part at buffer offset reverse to-binary value 4
	...
	build-export: func [
		...
		loop sym-nb [
			append out int-to-bin/to-bin16 ordinal
			ordinal: ordinal + 1
		]
	...
	build-section-reloc: func [
		...
		close-block: [
			change header int-to-bin/to-bin32 base
			change at header 5 int-to-bin/to-bin32 length? header	;-- store header + buffer size
		]
		...
		foreach offset refs [
			...
			append buffer int-to-bin/to-bin16 (offset - block) or type
		]
		do close-block
	...
	build-opt-header: func [job [object!] /local oh code-page code-base ep entry flags data-rva crodata-rva][
		...
		ep: switch/default job/type [
			...
			either external-linker/crt-entry [
				code-base + external-linker/crt-entry
			][
		...
		oh/TLS-addr:			external-linker/pe-tls-rva? data-rva crodata-rva
		oh/TLS-size:			external-linker/pe-tls-size?
	...
	icon-number?: func [icons [block!] /local data num][
		num: 0
		foreach icon icons [
			data: either binary? icon [icon][read/binary icon]
	...
	build-res-group-icon: func [
		...
		foreach icon icons [
			...
			repeat i nb [
				...
				change skip tail buf -2 int-to-bin/to-bin16 n
				n: n + 1
				p: skip p 16
			]
	...
	build-res-string: func [buf [binary!] key [string!] value [string!] /local str][
		...
		change buf int-to-bin/to-bin16 length? buf
		pad4 buf
	]
	...
	build-res-version: func [
		...
		entry/size: length? data-buf
		change data-buf int-to-bin/to-bin16 length? data-buf
		append out form-struct entry
	]
	...
	build: func [job [object!] /local page out pad code-ptr import-pos][
		...
		external-linker/crodata-base: all [
			find job/sections 'crodata
			base-address + section-addr?/memory job 'crodata
		]
		external-linker/cafter-base: all [
			find job/sections 'cafter
			base-address + section-addr?/memory job 'cafter
		]
		external-linker/apply-relocs job				;-- patch optional external C object relocations
