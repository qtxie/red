...
system-target-ARM64: make target-class [
	target:             'ARM64
	little-endian?:     yes
	struct-align-size:  8
	ptr-size:           8
	...
	unsupported-port-io: does [
		compiler-api/throw-error "ARM64 port I/O is not supported"
	]
	...
	emit-mov-imm32: func [value [integer! char!] /reg rd [integer!] /local v][
		...
		;-- Red's SHIFT uses a positive count for a right shift. The Rebol
		;-- host used -16 here, which made Red duplicate every low halfword.
	...
	emit-global-address: func [name [word! object! block!] reg [integer!] /local spec][
		...
		unless spec [compiler-api/throw-error ["unknown ARM64 symbol:" mold name]]
	...
	apple-aarch64?: does [(compiler-api/job-value 'ABI) = 'apple-aarch64]
	...
	stack-type-size: func [type [block!] /local size][
		...
		case [
			...
			compiler-api/any-float? type [either float32-type? type [4][8]]
	...
	emit-load-global: func [name [word! object! block!] type [block!] /reg rd [integer!] /local opcode][
		rd: any [rd 0]
		if compiler-api/any-float? type [emit-load-float-global/reg name type rd exit]
		emit-global-address name 16
		opcode: switch/default type/1 [
			byte!       [#{39400200}]
			logic!      [#{39400200}]
			...
			struct!     [#{F9400200}]
			union!      [#{F9400200}]
		][compiler-api/throw-error ["ARM64 global load type is not implemented:" mold type/1]]
		emit-i32 (opcode-int opcode) or rd
	]

	emit-store-global: func [name [word! object! block!] type [block!] /local opcode][
		if compiler-api/any-float? type [emit-store-float-global name type exit]
		emit-global-address name 16
		opcode: switch/default type/1 [
			byte!       [#{39000200}]
			logic!      [#{39000200}]
			...
			struct!     [#{F9000200}]
			union!      [#{F9000200}]
		][compiler-api/throw-error ["ARM64 global store type is not implemented:" mold type/1]]
		emit-i32 opcode
	]
	...
	emit-register-offset: func [
		...
		emit-mov-imm64/reg (absolute offset) 'uint64! scratch
	...
	emit-sp-offset-address: func [
		...
		emit-mov-imm64/reg (absolute offset) 'uint64! amount
		opcode: either negative? offset [#{CB000000}][#{8B000000}]
	...
	emit-load-local: func [name [word!] type [block!] /reg rd [integer!] /local offset opcode][
		...
		unless offset [compiler-api/throw-error ["unknown ARM64 local:" name]]
		if compiler-api/any-float? type [
		...
		opcode: switch/default type/1 [
			...
			struct!     [#{F8400000}]
			union!      [#{F8400000}]
		][compiler-api/throw-error ["ARM64 local load type is not implemented:" mold type/1]]
	...
	emit-store-local: func [name [word!] type [block!] /local offset opcode][
		...
		opcode: switch/default type/1 [
			...
			struct!     [#{F8000000}]
			union!      [#{F8000000}]
		][compiler-api/throw-error ["ARM64 local store type is not implemented:" mold type/1]]
	...
	emit-load: func [value /with cast [object!] /local raw raw-type local-spec type info][
		...
		case [
			...
			object? value [
				...
				case [
					all [compiler-api/any-float? type any [float? raw issue? raw]] [
			...
			logic? value [
				emit-load-integer either value [1][0]
			...
			word? value [
				raw-type: compiler-api/get-type value
				type: compiler-api/resolve-aliased raw-type
				either emitter/local-offset? value [
					either any [
					find by-value-args value
					all [block? raw-type 'value = last raw-type]
					all [compiler-api/locals local-spec: select compiler-api/locals value 'value = last local-spec]
			...
			'else [
			raw: compiler-api/unbox value
			compiler-api/throw-error ["ARM64 load is not implemented for:" mold raw]
			]
		]
	]
	...
	emit-argument: func [arg fspec [block!] /local value arg-type][
		...
		if block? value [value: <last>]
		arg-type: compiler-api/resolve-aliased compiler-api/get-type arg
		either compiler-api/any-float? arg-type [
			emit-load arg
			emit-i32 #{D10043FF}                              ; SUB sp, sp, #16
	...
	emit-call-syscall: func [
		...
		n: call-arg-index
		max-args: either (compiler-api/job-value 'syscall) = 'Linux [6][8]
		if n > max-args [compiler-api/throw-error ["ARM64 syscall with too many args:" n]]
	...
	emit-call-native: func [
		...
		either routine-call [
			...
			unless path-target? [
				target-type: compiler-api/resolve-aliased compiler-api/get-type target
	...
	emit-store: func [
		...
		unless last-value? value [
			...
			case [
				...
				true [
					source-type: compiler-api/resolve-aliased compiler-api/get-type value
					emit-load value
					case [
						all [type/1 = 'int64! compiler-api/signed-integer? source-type not wide-type? source-type][
							emit-i32 #{93407C00}           ; SXTW x0, w0
						]
						all [float32-type? type compiler-api/any-float? source-type not float32-type? source-type][
							emit-i32 #{1E624000}               ; FCVT s0, d0
						]
						all [compiler-api/any-float? type not float32-type? type float32-type? source-type][
	...
	emit-load-float-alt: func [
		...
		raw: compiler-api/unbox/deep value
		if object? value [
			source-type: compiler-api/resolve-aliased compiler-api/get-type value/data
		]
		source-single?: float32-type? source-type
		target-single?: float32-type? type
		if compiler-api/integer-type? source-type [
			wide?: compiler-api/int64? source-type
			case [
				integer? raw [
					either wide? [
				...
				word? raw [
					either emitter/local-offset? raw [
						...
						unless select emitter/symbols raw [
							raw: compiler-api/resolve-ns raw
						]
				...
				true [compiler-api/throw-error ["ARM64 alternate integer-to-float load is not implemented for:" mold raw]]
			]
			emit-int-to-float/reg source-type type 1
			exit
		]
		case [
			any [float? raw issue? raw] [
			...
			true [compiler-api/throw-error ["ARM64 alternate float load is not implemented for:" mold raw]]
		]
		emit-float-register-cast source-type type 1
	]
	...
	emit-int-to-float: func [
		...
		from-type: compiler-api/resolve-aliased from-type
		to-type: compiler-api/resolve-aliased to-type
		wide?: compiler-api/int64? from-type
		single?: float32-type? to-type
		signed-cast?: compiler-api/signed-integer? from-type
		opcode: either wide? [
			either signed-cast? [
	...
	emit-float-operation: func [
		...
		right-type: compiler-api/resolve-aliased compiler-api/resolve-expr-type args/2
		...
		right-source-type: either object? args/2 [
			compiler-api/resolve-aliased compiler-api/get-type args/2/data
		][right-type]

		if all [
			nested-right?
			object? args/2
			compiler-api/integer-type? source-type: compiler-api/resolve-expr-type args/2/data
		][
		...
		if all [
			nested-left?
			object? args/1
			compiler-api/integer-type? left-source-type
		][
			emit-int-to-float left-source-type op-type
		]
		if all [nested-left? compiler-api/any-float? left-source-type][
			emit-float-register-cast left-source-type op-type 0
		]
		if all [nested-right? compiler-api/any-float? right-source-type][
		...
		unless nested-left? [
			emit-load args/1
			if compiler-api/any-float? type [emit-float-register-cast type op-type 0]
		]
		...
		either find comparison-op name [
			...
			unless opcode [compiler-api/throw-error ["ARM64 float op is not implemented:" mold name]]
			emit-i32 opcode
		]
	]
	...
	emit-load-memory: func [type [block!] base offset [integer!] /local opcode][
		...
		opcode: switch/default type/1 [
			...
			float!      [#{FC400000}]
			float64!    [#{FC400000}]
		][compiler-api/throw-error ["ARM64 memory load type is not implemented:" mold type/1]]
	...
	emit-store-memory: func [type [block!] base offset [integer!] /local opcode value-reg][
		...
		opcode: switch/default type/1 [
			...
			float!      [#{FC000000}]
			float64!    [#{FC000000}]
		][compiler-api/throw-error ["ARM64 memory store type is not implemented:" mold type/1]]
	...
	emit-move-path-alt: func [/pair /with type [block!] /local resolved][
		resolved: compiler-api/resolve-aliased/silent any [type compiler-api/last-type]
		either all [resolved compiler-api/any-float? resolved] [
	...
	emit-store-path: func [
		...
		if all [
			...
			hfa: homogeneous-aggregate? compiler-api/last-type
			...
			type: compiler-api/resolve-type/with path/2 base-spec
			if all [block? type 'value = last type alias: compiler-api/find-aliased type/1][
				type: append copy alias 'value
			]
			...
			either hfa/1 [
				...
				aggregate-size: emitter/size-of? compiler-api/last-type
				repeat i slots [
		...
		unless last-value? value [
			...
			emit-i32 #{F90003E0}                              ; STR x0, [sp]
			emit-load value
			emit-move-path-alt/with either object? value [value/type][compiler-api/last-type]
		...
		switch kind [
			pointer! c-string! [
				full: either parent [compiler-api/resolve-type/with path/1 parent][compiler-api/resolve-type path/1]
				pointee: either kind = 'c-string! [[byte!]][full/2]
				emit-pointer-index-address path/2 pointee
				emit-store-memory pointee 0 0
			]
			struct! union! [
				unless base-spec [base-spec: second compiler-api/resolve-type to word! path/1]
	...
	emit-store-union-tag: func [spec [block!] name [word!] reg [integer!] /local id tag type opcode][
		if all [compiler-api/tagged-union? spec id: compiler-api/union-variant-id? spec name][
	...
	emit-push-struct: func [
		...
		aggregate-type: compiler-api/resolve-aliased any [type compiler-api/last-type]
	...
	emit-push-struct-ref: func [slots [integer!] /local offset i slot-offset][
		if call-struct-temp-slots < slots [
			compiler-api/throw-error "ARM64 struct argument temporary stack space was not reserved"
		]
	...
	classify-hfa-fields: func [
		...
		if union? [members: compiler-api/union-members members]
		kind: none
		count: 0
		valid?: yes
	...
	emit-divide-by-zero-check: func [wide? [logic!] /local spec][
		unless (compiler-api/job-value 'debug?) [exit]
	...
	emit-load-alt: func [
		...
		raw: compiler-api/unbox/deep value
		source-type: all [object? value compiler-api/resolve-aliased compiler-api/get-type value/data]
		if char? raw [raw: to integer! raw]
		if logic? raw [raw: either raw [1][0]]
		if all [
			...
			any [
				...
				all [
					object? value
					compiler-api/any-float? source-type
				]
			]
		][
			if float? raw [source-type: [float!]]
			single?: float32-type? source-type
			case [
				any [float? raw issue? raw] [emit-load-float-literal/reg raw source-type 1]
				word? raw [
					either emitter/local-offset? raw [
						emit-load-local/reg raw source-type 1
					][
				...
				true [compiler-api/throw-error ["ARM64 alternate float cast is not implemented for:" mold raw]]
			]
			wide?: any [compiler-api/int64? type compiler-api/any-pointer? type]
			signed-cast?: compiler-api/signed-integer? type
			opcode: either wide? [
				either signed-cast? [
		...
		case [
			...
			issue? raw [
				info: compiler-api/int64-literal-info raw
				unless info [compiler-api/throw-error ["ARM64 alternate issue literal is not implemented:" mold raw]]
			...
			true [compiler-api/throw-error ["ARM64 alternate load is not implemented for:" mold raw]]
		]
		case [
			type/1 = 'logic! [
			...
			compiler-api/integer-type? type [
				...
				if all [wide-type? type compiler-api/signed-integer? load-type not wide-type? load-type][
					emit-i32 #{93407C21}                          ; SXTW x1, w1
				]
			]
			all [compiler-api/any-pointer? type compiler-api/signed-integer? load-type not wide-type? load-type][
				emit-i32 #{93407C21}                          ; SXTW x1, w1
			]
		]
	]
	...
	emit-integer-operation: func [
		...
		pointer-op?: all [
			...
			not compiler-api/any-pointer? right-type
		]
		if pointer-op? [
		...
		unless imm? [
			...
			if pointer-op? [
				unless compiler-api/int64? right-type [emit-i32 #{93407C21}] ; SXTW x1, w1
				if scale > 1 [
					emit-mov-imm32/reg scale 16
					emit-i32 #{9B107C21}                       ; MUL x1, x1, x16
				]
		...
		case [
			...
			true [
				unless imm? [
					if all [compiler-api/overflow-check? name = left-shift-sym width = 4][
					...
					unless opcode [compiler-api/throw-error ["ARM64 integer op is not implemented:" mold name]]
					emit-i32 opcode
				]
				]
			]
		]
		if all [compiler-api/overflow-check? name = left-shift-sym][
			either width = 4 [
	...
	condition-code: func [op [word!] /local row][
		...
		if any [not row row/2 = '-][compiler-api/throw-error ["ARM64 condition is not implemented:" mold op]]
	...
	encode-branch: func [delta [integer!] cond [integer! none!] /local imm][
		unless zero? delta // 4 [compiler-api/throw-error "unaligned ARM64 branch"]
		imm: delta / 4
		either cond [
			if any [imm < -262144 imm > 262143][compiler-api/throw-error "ARM64 conditional branch is out of range"]
			(to integer! #{54000000}) or ((imm and 524287) * 32) or cond
		][
			if any [imm < -33554432 imm > 33554431][compiler-api/throw-error "ARM64 branch is out of range"]
	...
	encode-adr: func [delta [integer!] reg [integer!] /local encoded][
		if any [delta < -1048576 delta > 1048575][
			compiler-api/throw-error "ARM64 ADR target is out of range"
		]
	...
	emit-branch: func [
		...
		if all [cond any [distance < -1048576 distance > 1048572]][
			bytes: make binary! 8
			append bytes int-to-bin/to-bin32 encode-branch 8 (cond xor 1)
			far-distance: either back? [distance - 4][distance]
			append bytes int-to-bin/to-bin32 encode-branch far-distance none
			insert any [all [back? tail code] code] bytes
			return 8
		]
		insert any [all [back? tail code] code] int-to-bin/to-bin32 encode-branch distance cond
		4
	]
	...
	patch-jump-point: func [
		...
		change/part at buffer ptr int-to-bin/to-bin32 encode-branch exit-point - ptr cond 4
	]

	patch-jump-back: func [buffer [binary!] offset [integer!]][
		change/part at buffer offset int-to-bin/to-bin32 encode-branch negate (offset - 1) none 4
	]
	...
	emit-save-last: does [
		...
		either compiler-api/any-float? compiler-api/last-type [
			...
			saved-last-float32?: float32-type? compiler-api/last-type
			emit-i32 either saved-last-float32? [#{BC0003E0}][#{FC0003E0}] ; STUR s0/d0, [sp]
		][
			saved-last-float?: no
			saved-last-wide?: wide-type? compiler-api/resolve-aliased compiler-api/last-type
			emit-i32 #{F90003E0}                               ; STR x0, [sp]
		]
	]
	...
	emit-not: func [value /local raw type][
		...
		either type/1 = 'logic! [
			...
			unless compiler-api/integer-type? type [
				compiler-api/throw-error ["ARM64 NOT type is not implemented:" mold type/1]
			]
	...
	emit-prolog: func [
		...
		external?: to logic! all [fspec compiler-api/external-abi-call? fspec]
		...
		parse locals [
			...
			any [
				set arg-name word! set arg-type block! (
					resolved: compiler-api/resolve-aliased arg-type
					...
					if apple-external? [
						...
						either compiler-api/any-float? resolved [
							either fp-reg < 8 [
	...
	emit-epilog: func [
		...
		if slots [
			...
			either ret-ptr? [
				...
				ret-type: select locals compiler-api/return-def
	...
	patch-call: func [code-buf rel-ptr dst-ptr][
		change/part at code-buf rel-ptr
			int-to-bin/to-bin32 ((to integer! #{94000000}) or (((dst-ptr - rel-ptr) / 4) and 67108863))
			4
	]
	...
	on-global-epilog: func [runtime? [logic!] type [word!]][
		unless runtime? [
			either all [(compiler-api/job-value 'runtime?) (compiler-api/job-value 'need-main?)][
	...
	emit-casting: func [
		...
		from-type: compiler-api/resolve-aliased compiler-api/get-type value/data
		to-type: compiler-api/resolve-aliased value/type
		from-wide?: any [compiler-api/int64? from-type compiler-api/any-pointer? from-type]
		to-wide?: any [compiler-api/int64? to-type compiler-api/any-pointer? to-type]
		case [
			all [
				value/keep?
				to-type/1 = 'float32!
			...
			all [
				to-type/1 = 'logic!
				any [compiler-api/integer-type? from-type compiler-api/any-pointer? from-type]
			][
			...
			all [compiler-api/integer-type? to-type compiler-api/integer-type? from-type][
				emit-normalize-fixed-int-result/reg to-type index
				if all [to-type/1 = 'int64! compiler-api/signed-integer? from-type not from-wide?][
					emit-i32 (opcode-int #{93407C00}) or register-bits ; SXTW xN, wN
				]
			]
			all [
				compiler-api/any-pointer? to-type
				compiler-api/signed-integer? from-type
				not from-wide?
			][
				emit-i32 (opcode-int #{93407C00}) or register-bits ; SXTW xN, wN
			]
			all [compiler-api/any-float? to-type compiler-api/integer-type? from-type][
				single?: float32-type? to-type
				signed-cast?: compiler-api/signed-integer? from-type
			...
			all [compiler-api/integer-type? to-type compiler-api/any-float? from-type][
				single?: float32-type? from-type
				signed-cast?: compiler-api/signed-integer? to-type
			...
			all [float32-type? to-type compiler-api/any-float? from-type not float32-type? from-type][
				emit-i32 (opcode-int #{1E624000}) or register-bits ; FCVT sN, dN
			]
			all [compiler-api/any-float? to-type float32-type? from-type not float32-type? to-type][
				emit-i32 (opcode-int #{1E22C000}) or register-bits ; FCVT dN, sN
			]
			all [to-type/1 = 'logic! compiler-api/any-float? from-type][
	...
	patch-sub-call: func [buffer [binary!] ptr [integer!] offset [integer!] /local delta][
		delta: negate offset
		unless zero? delta // 4 [compiler-api/throw-error "unaligned ARM64 subroutine call"]
		if any [delta < -134217728 delta > 134217724][
			compiler-api/throw-error "ARM64 subroutine call is out of range"
		]
		change/part at buffer ptr
			int-to-bin/to-bin32 ((opcode-int #{94000000}) or ((delta / 4) and 67108863))
			4
	]
	emit-alt-last: :noop
	...
	emit-access-register: func [reg [word!] set? [logic!] value /local name kind number opcode][
		...
		kind: first name
		unless find "xw" kind [
			compiler-api/throw-error ["ARM64 system/cpu register is invalid:" reg]
		]
		...
		case [
			...
			options [
				...
				set [bits bit] switch/default option [
					...
					flush-to-zero [1 24]
					NaN-mode      [1 25]
				][compiler-api/throw-error ["invalid ARM64 FPU option name:" option]]
			...
			masks [
				...
				bit: switch/default mask [
					...
					denormal    [15]
					invalid-op  [8]
				][compiler-api/throw-error ["invalid ARM64 FPU mask name:" mask]]
	...
	emit-fpu-set: func [
		...
		either cword [
			...
			if logic? raw [raw: either raw [1][0]]
			unless integer? raw [
				compiler-api/throw-error "ARM64 FPU option values must be integer literals"
			]
			emit-i32 #{D53B4400}                              ; MRS x0, FPCR
			case [
				options [
