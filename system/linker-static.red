...
#include %formats/COFF.red
#include %formats/ELF-obj.red
#include %formats/Mach-O-obj.red
#include %formats/libc-exports.red
#include %formats/win32-exports.red
#include %formats/mac-cxx-exports.red
#include %formats/mac-libsystem.red
#include %formats/crt-helpers.red

object-reader-protocol: [
	i32-le is-defined-external? is-undefined-external? load load-from-bin
	pc-bias reject-reason reloc-kind sec-align sec-assoc sec-base-kind
	sec-base-offset sec-comdat-key sec-comdat? sec-data sec-kind sec-link
	sec-name sec-relocs sec-selection sec-size sec-vmaddr set-sec-base
	sym-common-align sym-common-size sym-name sym-sect sym-value
	sym-weak-default sym-weak? u32-le
]

object-reader: context [
	i32-le: func [bin [binary!] pos [integer!]][0]
	is-defined-external?: func [sym [block!]][false]
	is-undefined-external?: func [sym [block!]][false]
	load: func [path [file!]][none]
	load-from-bin: func [bin [binary!] path][none]
	pc-bias: 0
	reject-reason: func [bin [binary!]]["unsupported object"]
	reloc-kind: func [type [integer!]]['unsupported]
	sec-align: func [section [block!]][1]
	sec-assoc: func [section [block!]][none]
	sec-base-kind: func [section [block!]]['none]
	sec-base-offset: func [section [block!]][0]
	sec-comdat-key: func [section [block!]][none]
	sec-comdat?: func [section [block!]][false]
	sec-data: func [section [block!]][#{}]
	sec-kind: func [section [block!]][none]
	sec-link: func [section [block!]][none]
	sec-name: func [section [block!]][""]
	sec-relocs: func [section [block!]][[]]
	sec-selection: func [section [block!]][none]
	sec-size: func [section [block!]][0]
	sec-vmaddr: func [section [block!]][0]
	set-sec-base: func [section [block!] kind [word!] offset [integer!]][none]
	sym-common-align: func [sym [block!]][0]
	sym-common-size: func [sym [block!]][0]
	sym-name: func [sym [block!]][""]
	sym-sect: func [sym [block!]][0]
	sym-value: func [sym [block!]][0]
	sym-weak-default: func [sym [block!]][none]
	sym-weak?: func [sym [block!]][false]
	u32-le: func [bin [binary!] pos [integer!]][0]
]
object-reader-defaults: make object-reader []

configure-object-reader: func [source [object!] /local name slot fallback][
	foreach name object-reader-protocol [
		slot: in source name
		fallback: in object-reader-defaults name
		set in object-reader name get any [slot fallback]
	]
]
...
static-link: context [
	...
	ptr-struct: virtual-struct/make-value [value [integer!]] none
	le32: func [n [integer!]][ptr-struct/value: n  virtual-struct/form-value ptr-struct]

	bit31: 0 - 2147483647 - 1		;-- 80000000h (the literal cannot be lexed)

	abort: func [msg [block!]][
		print rejoin ["*** Static linking error: " form reduce msg]
	...
	archive?: func [name [string!]][
		to logic! find [%.lib %.a] (suffix? to file! lowercase copy name)
	]
	...
	framework?: func [name [string!]][
		to logic! any [find name ".framework/"  find name ".framework\"]
	]
	...
	open-archive: func [
		...
		unless arc/index [							;-- BSD ar: no "/" symbol index
			...
			foreach m read-archive path [
				obj: object-reader/load-from-bin m/2 (rejoin [to-local-file path "(" m/1 ")"])
				unless obj [
					abort reduce [
						"unsupported archive member" m/1
						"in" to-local-file path "--" object-reader/reject-reason m/2
					]
				]
	...
	obj-defines?: func [obj [object!] nm [string!] /local sym existing][
		foreach sym obj/symbols [
			if all [
				object-reader/is-defined-external? sym
				nm = object-reader/sym-name sym
			][
				existing: select sym-addr nm
				either existing [
					if all [existing/3  not object-reader/sym-weak? sym][return true]
				][return true]
			]
		]
		false
	...
	pull-member: func [job [object!] arc off [integer!] /local obj m][
		either obj: select arc/pulled off [
			...
			obj: object-reader/load-from-bin m/2 (rejoin [to-local-file arc/path "(" m/1 ")"])
			unless obj [
				abort reduce [
					"unsupported archive member" m/1
					"in" to-local-file arc/path "--" object-reader/reject-reason m/2
				]
			]
		...
		either arc/index [
			...
			foreach obj arc/eager [
				foreach sym obj/symbols [
					if all [
						object-reader/is-defined-external? sym
						seed-match? object-reader/sym-name sym
					][
						note-merged-object obj
						mark-live-sections obj
						merge-sections job obj
	...
	register-weak-aliases: func [obj [object!] /local sym nm dn][
		foreach sym obj/symbols [
			if all [
				object-reader/sym-weak? sym
				dn: object-reader/sym-weak-default sym
			][
				nm: object-reader/sym-name sym
				unless alias-of nm [
		...
		parse text [
			any [
				...
				| copy tok [some [{"} thru {"} | non-space]] (
					...
					case [
						parse low ["/include:" to end][
							val: copy skip tok 9
							unless any [empty? val  find needed val][
								append needed val
							]
						]
						parse low ["/alternatename:" to end][
	...
	finalize-crt-sections: func [
		...
		foreach [name section obj] crt-sections [
			a: object-reader/sec-align section
			if a > max-align [max-align: a]
			pad-to data a
			base: length? data
			append data object-reader/sec-data section
			object-reader/set-sec-base section 'data base
	...
	finalize-pe-tls-sections: func [
		...
		foreach [name section obj] tls-pe-sections [
			...
			if all [
				ckey: object-reader/sec-comdat-key section
	...
	finalize-elf-cpp: func [
		...
		foreach [seq obj section] eh-frames [
			either obj-format = 'Mach-o [
				...
				append ehframe-buf object-reader/sec-data section
				object-reader/set-sec-base section 'eh-frame base
			][
		...
		unless empty? exidx-sections [
			...
			foreach [obj section] exidx-sections [
				link: object-reader/sec-link section
				...
				either all [txt  'code = object-reader/sec-base-kind txt][
					repend/only sorted [object-reader/sec-base-offset txt  obj section]
				][
					poke section 7 'none
				]
			]
		...
		unless empty? tls-sections [
			...
			foreach [seq obj section] tls-sections [
				if seq = 0 [
					pad-to data object-reader/sec-align section
					append data object-reader/sec-data section
					object-reader/set-sec-base section 'tls ((length? data) - etls-off - object-reader/sec-size section)
					unless find objs obj [append objs obj]
				]
			]
			...
			foreach [seq obj section] tls-sections [
				if seq = 1 [
					base: etls-memsz
					base: base + ((object-reader/sec-align section) - 1)
					base: base - (base // object-reader/sec-align section)
					object-reader/set-sec-base section 'tls base
					etls-memsz: base + object-reader/sec-size section
					unless find objs obj [append objs obj]
				]
			]
		]
	...
	merge: func [
		...
		configure-object-reader reader
	...
	mark-section-live?: func [obj [object!] sect [integer!] /local section][
		...
		unless object-reader/sec-kind section [return false]
	...
	mark-live-sections: func [
		...
		foreach section obj/sections [
			idx: idx + 1
			if all [
				object-reader/sec-kind section
				any [
					not object-reader/sec-comdat? section
					...
					find [init-array eh-frame arm-exidx crt tls-data tls-bss] object-reader/sec-kind section
				]
			][
		...
		foreach sym obj/symbols [
			if all [
				object-reader/is-defined-external? sym
				any [
					find needed object-reader/sym-name sym
					seed-match? object-reader/sym-name sym
				]
			][
				mark-section-live? obj object-reader/sym-sect sym
			]
		]
		until [
			changed?: false
			...
			foreach section obj/sections [
				...
				if all [
					...
					sect: object-reader/sec-assoc section
					sect > 0
			...
			foreach section obj/sections [
				if section-live? section [
					foreach r object-reader/sec-relocs section [
						target: pick obj/symbols (r/2 + 1)
						if target [
							sect: object-reader/sym-sect target
							if sect > 0 [
						...
						if all [
							(length? r) >= 4
							'sectdiff = object-reader/reloc-kind r/3
						][
							target2: pick obj/symbols (r/4 + 1)
							if target2 [
								sect: object-reader/sym-sect target2
								if sect > 0 [
									if mark-section-live? obj sect [changed?: true]
								]
							]
	...
	note-undefined: func [obj [object!] /local section sym nm sec-kind][
		foreach section obj/sections [
			sec-kind: object-reader/sec-base-kind section
			if all [
				sec-kind = 'none
				'crt = object-reader/sec-kind section
			...
			unless sec-kind = 'none [
				foreach r object-reader/sec-relocs section [
					...
					if all [sym  object-reader/is-undefined-external? sym][
						nm: object-reader/sym-name sym
		...
		foreach sym obj/symbols [
			if all [sym  object-reader/is-undefined-external? sym][
				...
				unless find needed nm [append needed copy nm]
				if obj-format = 'ELF [
					either object-reader/sym-weak? sym [
	...
	merge-sections: func [
		...
		merged?: false
		foreach section obj/sections [
			kind: object-reader/sec-kind section
			...
			if all [
				obj-format = 'ELF
				kind = 'rdata
				".eh_frame" = object-reader/sec-name section
			][
				kind: 'eh-frame
			]
			...
			if all [
				...
				sect > 0
				sect <= length? obj/sections
				pkey: object-reader/sec-comdat-key pick obj/sections sect
			...
			if kind [
				...
				either all [
					...
					if all [
						6 = object-reader/sec-selection section
						(object-reader/sec-size section) > entry/3
					][
						print [
							"*** Warning: (largest) COMDAT" ckey "keeps its first,"
							"smaller copy:" entry/3 "vs" object-reader/sec-size section "bytes"
						]
					]
					kind: none
					ckey: none
			...
			case [
				kind = 'code [
					pad-to code a
					base: length? code
					append code object-reader/sec-data section
					object-reader/set-sec-base section 'code base
				...
				kind = 'rdata [
					...
					either obj-format = 'PE [
						...
						append crodata-buf object-reader/sec-data section
						object-reader/set-sec-base section 'crodata base
					][
						pad-to data a
						base: length? data
						append data object-reader/sec-data section
				...
				kind = 'cafter [
					...
					either obj-format = 'PE [
						...
						append cafter-buf object-reader/sec-data section
						object-reader/set-sec-base section 'cafter base
					][
						pad-to data a
						base: length? data
						append data object-reader/sec-data section
				...
				kind = 'tls [
					...
					repend tls-pe-sections [copy object-reader/sec-name section  section  obj]
					poke section 11 true
					merged?: true
				]
				kind = 'bss [
					pad-to data a
					base: length? data
					insert/dup tail data null object-reader/sec-size section
					object-reader/set-sec-base section 'data base
					poke section 11 true
					merged?: true
				]
				kind = 'crt [
					...
					repend crt-sections [copy object-reader/sec-name section  section  obj]
					poke section 11 true
					merged?: true
				]
				kind = 'init-array [
					...
					repend init-arrays [
						cpp-arrival obj object-reader/sec-name section  obj  section
					]
					poke section 11 true
					merged?: true
				]
			...
			if all [
				...
				repend comdat-keys [
					copy ckey
					reduce [
						object-reader/sec-base-kind section
						object-reader/sec-base-offset section
						object-reader/sec-size section
						obj
						copy object-reader/sec-name section
					]
				]
			]
		]
		idx: 0
		foreach sym obj/symbols [
			sect: object-reader/sym-sect sym
			common-live?: false
			if all [
				obj-format = 'PE
				sect = 0
				0 < object-reader/sym-value sym
			][
				foreach section obj/sections [
					sec-kind: object-reader/sec-base-kind section
					unless sec-kind = 'none [
	...
	register-symbol: func [
		...
		if all [
			...
			name: object-reader/sym-name sym
			unless select sym-addr name [
				data: job/sections/data/2
				pad-to data 4
				data-off: length? data
				insert/dup tail data null object-reader/sym-value sym
		...
		if sect <= 0 [exit]
		section: pick obj/sections sect
		if 'none = object-reader/sec-base-kind section [exit]	;-- dropped section, or deferred and not yet laid out
		kind: either (object-reader/sec-kind section) = 'tls ['tls][object-reader/sec-base-kind section]
		base: object-reader/sec-base-offset section
		name: object-reader/sym-name sym
		new-weak: object-reader/sym-weak? sym
		either existing: select sym-addr name [
			...
			if all [existing/3  not new-weak][
				existing/1: kind
				existing/2: base + object-reader/sym-value sym
				existing/3: false
			]
		][
			repend sym-addr [
				name reduce [kind  base + object-reader/sym-value sym  new-weak]
			]
		]
	]
		...
		foreach [path obj] objects [
			...
			foreach section obj/sections [
				...
				unless sec-kind = 'none [
					foreach r object-reader/sec-relocs section [
						sym: pick obj/symbols (r/2 + 1)
						if all [sym  object-reader/is-undefined-external? sym  not find pending sym][
			...
			foreach sym pending [
					...
					unless any [
						...
						case [
							...
							all [obj-format = 'PE  0 < object-reader/sym-value sym][
								pad-to data 4
								data-off: length? data
								insert/dup tail data null object-reader/sym-value sym
								repend sym-addr [name reduce ['data data-off false]]
	...
	append-linux-helper-lib: func [out [block!] path [string!] /local file][
		file: to-red-file path
		if all [
			not empty? path
			exists? file
			elf-archive? file
	...
	merge-commons: func [
		...
		foreach [path obj] objects [
			foreach sym obj/symbols [
				sz: object-reader/sym-common-size sym
				if sz > 0 [
					nm: object-reader/sym-name sym
					a: object-reader/sym-common-align sym
	...
	rewrite-macho-ehframe: func [
		...
		map-va: func [obj [object!] t [integer!] /local s][
			foreach s obj/sections [
				if all [
					'none <> object-reader/sec-base-kind s
					t >= object-reader/sec-vmaddr s
					t < ((object-reader/sec-vmaddr s) + object-reader/sec-size s)
				][
					return (object-reader/sec-base-offset s) + (t - object-reader/sec-vmaddr s)
						+ switch/default object-reader/sec-base-kind s [
							code	 [code-va]
							data	 [data-va]
							rodata	 [rodata-va]
							eh-frame [ehframe-base]
						][abort reduce ["eh_frame pcrel target in unsupported section kind:" object-reader/sec-base-kind s]]
				]
			]
			abort reduce ["eh_frame pcrel target outside every live section:" t]
		]
		foreach [seq obj section] eh-frames [
			cb0:	object-reader/sec-base-offset section	;-- chunk offset in ehframe-buf
			sec-va:	object-reader/sec-vmaddr section		;-- chunk address in the OBJECT layout
			size:	object-reader/sec-size section
		...
		foreach [obj section] nlptr-sections [
			...
			unless empty? fills [
				foreach sec obj/sections [
					if 'code = object-reader/sec-base-kind sec [
						sec-base: object-reader/sec-base-offset sec
						relocs-blk: object-reader/sec-relocs sec
						keep: copy []
						foreach r relocs-blk [
							either all [
								r/3 = 8
								sym: pick obj/symbols (r/2 + 1)
								"" = object-reader/sym-name sym
								nl-idx = object-reader/sym-sect sym
								fr: select/only fills reduce [
									slot: object-reader/i32-le code (sec-base + r/1 + 1)
								]
							][
				...
				relocs-blk: object-reader/sec-relocs section
	...
	wire-imp-relocs: func [
		...
		foreach [path obj] objects [
			foreach section obj/sections [
				sec-kind: object-reader/sec-base-kind section
				unless sec-kind = 'none [
					sec-base: object-reader/sec-base-offset section
					remove-each r object-reader/sec-relocs section [
						sym: pick obj/symbols (r/2 + 1)
						either all [sym  imp: select imp-table object-reader/sym-name sym][
							either sec-kind = 'code [
								...
								abort reduce [
									...
									object-reader/sym-name sym "(in" path ")"
								]
							]
						][false]
					]
	...
	find-helper-libs: func [/local out root libdir ver p lib][
		...
		if root [
			libdir: rejoin [to-red-file root %Lib/]
	...
	find-msvc-lib-dirs: func [/local out root ver dir sub][
		...
		if exists? vswhere-path [
			...
			unless error? try [
				...
				if all [
					not empty? root
					ver: newest-subdir dir: join to-red-file root %/VC/Tools/MSVC/
				][
		...
		if root: reg-read "HKLM\SOFTWARE\Microsoft\Windows Kits\Installed Roots" "KitsRoot10" [
			dir: rejoin [to-red-file root %Lib/]
	...
	resolve-reloc-target: func [
		...
		target-info: either any [
			object-reader/is-defined-external? sym
			object-reader/is-undefined-external? sym
			object-reader/sym-weak? sym
			all [obj-format = 'Mach-o  0 < object-reader/sym-common-size sym]
		][
			resolve-sym-addr object-reader/sym-name sym
		][none]
		either target-info [
			target-info
		][
			tsect: object-reader/sym-sect sym
			either all [tsect > 0  tsect <= length? obj/sections][
				tsection: pick obj/sections tsect
				tkind: object-reader/sec-base-kind tsection
				if tkind = 'none [
					...
					if all [
						ckey: object-reader/sec-comdat-key tsection
						entry: select comdat-keys ckey
					][
						tname: object-reader/sec-name tsection
						unless tname = entry/5 [
							foreach twin entry/4/sections [
								if all [
									ckey = object-reader/sec-comdat-key twin
									tname = object-reader/sec-name twin
									'none <> object-reader/sec-base-kind twin
								][
									return reduce [
										object-reader/sec-base-kind twin
										(object-reader/sec-base-offset twin) + object-reader/sym-value sym
										false
									]
								]
							]
						...
						return reduce [entry/1  entry/2 + object-reader/sym-value sym  false]
					]
					abort reduce [
							"relocation targets a dropped section:"
							object-reader/sym-name sym "->" object-reader/sec-name tsection
							"kind" object-reader/sec-kind tsection "in" path
						]
				]
				toff: (object-reader/sec-base-offset tsection) + object-reader/sym-value sym
				reduce [tkind toff false]
			][
				abort reduce ["unresolved symbol" object-reader/sym-name sym "in" path]
			]
		]
	]
	...
	allocate-got-slots: func [
		...
		foreach [path obj] objects [
			foreach section obj/sections [
				...
				unless sec-kind = 'none [
					foreach r object-reader/sec-relocs section [
						kind: object-reader/reloc-kind r/3
						if find [
			...
			foreach section obj/sections [
				...
				unless sec-kind = 'none [
					...
					foreach r object-reader/sec-relocs section [
						...
						addend:    object-reader/i32-le buf (patch-pos + 1)
						kind:      object-reader/reloc-kind r-type
						got-base:  either got-start = none [0][data-base + got-start]
						sym-name:  object-reader/sym-name sym
						...
						if all [obj-format = 'Mach-o  kind = 'pc32][
							addend: addend + r-va + object-reader/sec-vmaddr section
						]
						...
						case [
							...
							kind = 'pc32 [
								...
								change at buf (patch-pos + 1)
									le32 ((target-va + addend) - patch-va - object-reader/pc-bias)
							]
							kind = 'plt32 [
								patch-va: buf-base + patch-pos
								change at buf (patch-pos + 1)
							...
							kind = 'gotpc [
								patch-va: buf-base + patch-pos
								change at buf (patch-pos + 1)
									le32 ((got-base + addend) - patch-va - object-reader/pc-bias)
							]
							kind = 'gotoff [
								change at buf (patch-pos + 1)
							...
							kind = 'sectdiff [
								...
								min-section: pick obj/sections (object-reader/sym-sect sym)
								sub-section: pick obj/sections (object-reader/sym-sect sub-sym)
								orig-diff: ((object-reader/sec-vmaddr min-section) + min-offset)
									- ((object-reader/sec-vmaddr sub-section) + sub-offset)
								change at buf (patch-pos + 1)
									le32 (((target-va + min-offset) - sub-va) + (addend - orig-diff))
							]
							kind = 'arm-call [
								insn:     object-reader/u32-le buf (patch-pos + 1)
								patch-va: buf-base + patch-pos
								change at buf (patch-pos + 1)
