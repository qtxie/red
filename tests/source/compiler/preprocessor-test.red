Red [
	...
	Purpose: {
		Red port of the Rebol original, which stays in the tree until Phase C
		retires the Rebol entry points. Unlike %lexer-test.r, which drives the
		Rebol encapper's lexer, this one drives the *Red* preprocessor (#do,
		#if, #macro, #switch, #case, #reset and preprocessor/fetch-next), so it
		belongs in the Red suite.

		The directives live inside {...} payloads the compiler is handed as
		source strings; they are never preprocessed here.
	}
]
